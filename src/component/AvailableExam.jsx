...
import { toast } from "sonner";
import { Edit as EditIcon, Trash2, UserPlus } from "lucide-react";
import ConfirmModal from "./ConfirmModal";
import Pagination from "./Pagination";
...
const AvailableExam = () => {
  ...
  // ✅ FIXED: handle navigation safely
  useEffect(() => {
    if (message) {
          ...
          {paginatedData.map((data, index) => {
            ...
            return (
              <ExamRow key={data.examId}>
                ...
                <ExamCol>
                  <ButtonDiv>
                    <Edit onClick={() => navigate(`/edit-exam/${data.examId}`)}>
                      <EditIcon size={16} />
                    </Edit>
                  </ButtonDiv>
                </ExamCol>
